...
class Inpainting_GAN(object):
    ...
    def __init__(self, data_ob, config):
        ...
        self.iter_num = config.get_iter_num()
    ...
    def test(self):
        ...
        with tf.Session(config=config) as sess:
            sess.run(init)
            self.saver.restore(sess, os.path.join(self.write_model_path, 'model_{:06d}.ckpt'.format(self.test_step)))
            ...
            for j in range(self.batch_num):
                ...
                input_masks, test_incomplete_img, test_x_tilde, new_test_x_tilde = sess.run([self.input_masks,
                                                    self.incomplete_img, self.x_tilde, self.new_x_tilde], feed_dict=f_d)
    ...
    def train(self):
        ...
        with tf.Session(config=config) as sess:
            ...
            if ckpt and ckpt.model_checkpoint_path:
                ...
                try:
                    self.load_saver.restore(sess, os.path.join(self.read_model_path,
                                                               'model_{:06d}.ckpt'.format(self.iter_num)))
            ...
            while step <= self.max_iters:
                ...
                if np.mod(step, 2000) == 0:
                    ...
                    output_concat = np.concatenate([real_batch_image,train_output_img[2], train_output_img[3], train_output_img[4]], axis=0)
                    local_output_concat = np.concatenate([train_output_img[0],train_output_img[1], train_output_img[5], train_output_img[6]], axis=0)
                    test_output_concat = np.concatenate([real_test_batch,test_output_img[0], test_output_img[2], test_output_img[1]], axis=0)
