    ...
    def test(self):
        ...
        with tf.Session(config=config) as sess:
            ...
            for j in range(self.batch_num):
                ...
                test_output_concat = np.concatenate([real_test_batch, mask, test_incomplete_img,
                                                     test_recon_img, new_test_recon_img], axis=0)
    ...
    def train(self):
        ...
        with tf.Session(config=config) as sess:
            ...
            while step <= self.max_iters:
                ...
                real_batch_image_path, real_batch_image, real_eye_pos = \
                                                        sess.run([image_path,train_images, train_eye_pos])
                batch_masks, batch_left_eye_pos, batch_right_eye_pos = self.get_Mask_and_pos(real_eye_pos)
